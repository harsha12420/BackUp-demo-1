...
export class SidebarComponent implements OnInit {
  ...
  checkNavActiveOnLoad() {
    this.navServices.items.subscribe((menuItems: any) => {
      this.menuItems = menuItems.filter((items: any) =>
        items.role?.includes(this.userRole)
      );
      ...
      this.router.events.subscribe((event: any) => {
        if (event instanceof NavigationStart) {
          ...
          if (path[path.length - 2] !== eventUrl[eventUrl.length - 2]) {
            ...
            let subSidemenu2 = document.querySelectorAll(
              '.sub-side-menu__item2.active'
            );
    ...
    this.activeSubMenuOnLoad();
  }

  activeSubMenuOnLoad() {
    const event: any = this.router;
    this.menuItems.filter((items: any) => {
      if (items.path === event.url) {
        this.setNavActive(items);
      }
  ...
  // Toggle menu
  toggleNavActive(item: any, hasChild = true) {
    if (hasChild) window.scrollTo(0, 0)
  ...
  async ngOnInit() {
    ...
    const userRole = await this.encryptDataService.getAndDecryptData();
    if (userRole) {
