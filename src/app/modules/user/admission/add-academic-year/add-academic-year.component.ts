...
export class AddAcademicYearComponent {
  ...
  async onSubmit() {
    try {
      ...
      const response: any = await this.apiService.updateAcademicYear(obj);
      this.utils.hideLoading();
      if (response.statusCode === 200) {
  ...
  deleteAcademicYear(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const response: any = await this.apiService.deleteAcademicYear(id);
        if ((response.statusCode = 200)) {
          Swal.fire({
            title: 'Deleted!',
            text: 'Your academic year has been deleted.',
            icon: 'success',
            confirmButtonColor: '#6259ca',
          });
          await this.getList();
  ...
  async exportAcademicYear(exportType: string) {
    ...
    this.exportService.exportData(exportType, headerRow, data, `Academic Year`)
  }
}
