...
export class AddStateComponent {
  ...
  async onSubmit() {
    try {
      ...
      const obj = {
        title: this.addStateForm.controls['name'].value,
        isActive:
          this.addStateForm.controls['Status'].value == 'true' ? true : false,
      };
      this.EditId ? (obj['id'] = this.EditId) : '';
      const response: any = await this.apiService.addMasterState(obj);
      this.utils.hideLoading();
  ...
  async editState(listId) {
    ...
    const queryList = `?state_id=${listId}`;

    this.onModalOpen();
    const response: any = await this.apiService.getByIdMasterState(queryList);
    if ((response.statusCode = 200)) {
      this.addStateForm.controls['name'].setValue(
        response.data.result[0].state_name
      );

      this.addStateForm.controls['Status'].setValue(
        response.data.result[0].state_active.toString()
      );
    }
  }
  ...
  deleteState(id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        const response: any = await this.apiService.deleteMasterState(id);
        if ((response.statusCode = 200)) {
          Swal.fire({
            title: 'Deleted!',
